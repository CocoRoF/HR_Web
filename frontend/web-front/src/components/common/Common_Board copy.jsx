...
class Board extends Component {
    ...
    animation = () => {
        ...
        if (this.state.extendState) {
            ...
            const setAngle = (angleNow, n) => {
                if (this.notMoving) {
                    const absoluteAngle = 360 + (n*-24);
                    const absoluteNow = ((angleNow % 360) + 360) % 360;
                    const rotateNum = (angleNow / 360);

                    let leftDistance = absoluteAngle - absoluteNow;
                    let rightDistance = absoluteNow - absoluteAngle;
                    let moveTo = absoluteAngle + (rotateNum * 360);

                    if ((absoluteAngle - absoluteNow) >= 0) {
                        leftDistance += 360;
